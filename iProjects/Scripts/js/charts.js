$(document).ready(function(){        
    ...
    if($("#dash_chart_1").length > 0) {
        ...
        var chartData = [];
        $.get("/Projects/GetChartData", function(response) {
            for (var i = 0; i < response.length; i++) {
                chartData[i] = [];
                chartData[i][0] = response[i].CreationDate;
                chartData[i][1] = response[i].ProjectCount;
            }
        }).then(function() {
            var dash_chart_1 = $.plot($("#dash_chart_1"), [{ data: chartData, label: "Sales" }], {
